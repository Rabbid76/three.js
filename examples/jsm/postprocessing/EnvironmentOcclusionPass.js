...
class EnvironmentOcclusionPass extends Pass {
	...
	constructor( scene, camera, width, height, parameters, aoParameters, pdParameters ) {
		...
		this.gtaoMaterial.defines.BENT_NORMAL = 1;
		...
		this.pdMaterial.defines.SAMPLE_LUMINANCE = 'a.a';
		this.pdMaterial.defines.FRAGMENT_OUTPUT = 'denoised.rgba';
		...
		this.pdMaterial.uniforms.depthPhi.value = 1;
		this.pdMaterial.uniforms.normalPhi.value = 1;
		...
		this.blendMaterial = new ShaderMaterial( {
			defines: Object.assign( {}, GTAOBlendShader.defines ),
		...
		this.blendMaterial.defines.BLEND_MODE = 1;

		this.blendBentNormalMaterial = new ShaderMaterial( {
			defines: Object.assign( {}, GTAOBlendShader.defines ),
			uniforms: UniformsUtils.clone( GTAOBlendShader.uniforms ),
			vertexShader: GTAOBlendShader.vertexShader,
			fragmentShader: GTAOBlendShader.fragmentShader,
		...
		this.blendBentNormalMaterial.defines.BLEND_MODE = 2;
	...
	render( renderer, writeBuffer, readBuffer /*, deltaTime, maskActive */ ) {
		...
		if ( this.output != EnvironmentOcclusionPass.OUTPUT.Off ) {

			this.renderDebugPass( renderer, writeBuffer );

		}

	}

	renderDebugPass( renderer, writeBuffer ) {

		switch ( this.output ) {

			case EnvironmentOcclusionPass.OUTPUT.AO:

				this.blendMaterial.uniforms.tDiffuse.value = this.pdRenderTarget.texture;
				this.blendMaterial.blending = NoBlending;
				this.renderPass( renderer, this.blendMaterial, this.renderToScreen ? null : writeBuffer );

				break;

			case EnvironmentOcclusionPass.OUTPUT.BentNormal:

				this.blendBentNormalMaterial.uniforms.tDiffuse.value = this.pdRenderTarget.texture;
				this.renderPass( renderer, this.blendBentNormalMaterial, this.renderToScreen ? null : writeBuffer );

				break;

			case EnvironmentOcclusionPass.OUTPUT.Depth:
			...
			case EnvironmentOcclusionPass.OUTPUT.Normal:
			...
			default:
				console.warn( 'THREE.EnvironmentOcclusionPass: Unknown output type.' );
...
EnvironmentOcclusionPass.OUTPUT = {
	'Off': 0,
	'Depth': 2,
	'Normal': 3,
	'AO': 4,
...
export { EnvironmentOcclusionPass };
